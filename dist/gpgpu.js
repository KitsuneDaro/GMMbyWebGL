...
exports.GPGPU = exports.CreateGPGPU = void 0;
/*
    GPGPUのオブジェクトを作ります。
...
exports.CreateGPGPU = CreateGPGPU;
class GPGPU {
    /*
        GPGPUのコンストラクタ
    */
        ...
        if (pkg) {
            ...
            for (let varying of pkg.varyings) {
                if (varying.feedbackBuffer) {
                    ...
                    // バッファを削除する。
                    gl.deleteBuffer(varying.feedbackBuffer);
                    chk();
                }
            }
            if (pkg.transformFeedback) {
                // Transform Feedbackがある場合
                gl.deleteTransformFeedback(pkg.transformFeedback);
                chk();
            }
            // テクスチャのバインドを解く。
    ...
    */
    makeProgram(vertex_shader, fragment_shader, varyings) {
        ...
        if (varyings) {
            ...
            gl.transformFeedbackVaryings(prg, varying_names, gl.SEPARATE_ATTRIBS);
            chk(); // gl.INTERLEAVED_ATTRIBS 
        }
        // プログラムをリンクする。
        gl.linkProgram(prg);
        chk();
    ...
    */
    makeAttrib(pkg) {
        ...
        for (let attrib of pkg.attributes) {
            ...
            // attribute変数の位置
            attrib.AttribLoc = gl.getAttribLocation(pkg.program, attrib.name);
            chk();
            // 指定した位置のattribute配列を有効にする。
            gl.enableVertexAttribArray(attrib.AttribLoc);
            chk();
    ...
    */
    makeTexture(pkg) {
        ...
        for (var i = 0; i < pkg.textures.length; i++) {
            ...
            // テクスチャのuniform変数の位置
            tex_inf.locTexture = gl.getUniformLocation(pkg.program, tex_inf.name);
            chk();
            ...
            gl.activeTexture(this.TEXTUREs[i]);
            chk();
            // 作成したテクスチャをバインドする。
            gl.bindTexture(dim, tex_inf.Texture);
            chk();
    ...
    */
    setTextureData(pkg) {
        for (var i = 0; i < pkg.textures.length; i++) {
            ...
            gl.uniform1i(tex_inf.locTexture, i);
            chk();
            var dim = tex_inf.samplerType == "sampler3D" ? gl.TEXTURE_3D : gl.TEXTURE_2D;
            // 指定した位置のテクスチャをアクティブにする。
            gl.activeTexture(this.TEXTUREs[i]);
            ...
            if (tex_inf.value instanceof Image) {
                ...
                if (dim == gl.TEXTURE_2D) {
                    // 2Dのテクスチャの場合
                    // テクスチャのデータをセットする。
                    gl.texImage2D(gl.TEXTURE_2D, 0, internal_format, tex_inf.shape[1], tex_inf.shape[0], 0, format, gl.FLOAT, tex_inf.value);
                    chk();
                }
                else {
                    // 3Dのテクスチャの場合
                    ...
                    // テクスチャのデータをセットする。
                    gl.texImage3D(gl.TEXTURE_3D, 0, internal_format, tex_inf.shape[2], tex_inf.shape[1], tex_inf.shape[0], 0, format, gl.FLOAT, tex_inf.value);
                    chk();
                }
            }
        }
    ...
    makeVertexIndexBuffer(pkg, param) {
        ...
        chk();
        gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, param.VertexIndexBuffer, gl.STATIC_DRAW);
        chk();
    ...
    makePackage(param) {
        ...
        if (pkg.varyings.length != 0) {
            ...
            for (let varying of pkg.varyings) {
                ...
                varying.feedbackBuffer = gl.createBuffer();
                chk();
                // バッファをバインドする。
                gl.bindBuffer(gl.ARRAY_BUFFER, varying.feedbackBuffer);
                chk();
            ...
            pkg.transformFeedback = gl.createTransformFeedback();
            chk();
        }
    ...
    */
    setAttribData(pkg) {
        ...
        for (let attrib of pkg.attributes) {
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, attrib.AttribBuffer);
            chk();
            // 指定した位置のattribute変数の要素数(dim)と型(float)をセットする。
            gl.vertexAttribPointer(attrib.AttribLoc, dim, gl.FLOAT, false, 0, 0);
            chk();
    ...
    setUniformsData(pkg) {
        ...
        for (let u of pkg.uniforms) {
            if (u.value instanceof Float32Array) {
                ...
                switch (u.type) {
                    case "mat4":
                        gl.uniformMatrix4fv(u.locUniform, false, u.value);
                        chk();
                        break;
                    case "mat3":
                        gl.uniformMatrix3fv(u.locUniform, false, u.value);
                        chk();
                        break;
                    case "vec4":
                        gl.uniform4fv(u.locUniform, u.value);
                        chk();
                        break;
                    case "vec3":
                        gl.uniform3fv(u.locUniform, u.value);
                        chk();
                        break;
                    case "vec2":
                        gl.uniform2fv(u.locUniform, u.value);
                        chk();
                        break;
                    case "float":
                        gl.uniform1fv(u.locUniform, u.value);
                        chk();
                        break;
                    default:
                        assert(false);
                ...
                if (u.type == "int" || u.type == "bool") {
                    gl.uniform1i(u.locUniform, u.value);
                    chk();
                }
                else {
                    gl.uniform1f(u.locUniform, u.value);
                    chk();
                }
            }
        }
    ...
    */
    compute(param) {
        ...
        if (pkg.varyings.length == 0) {
            ...
            chk();
            // 頂点インデックスバッファをバインドする。
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, pkg.VertexIndexBufferInf.buffer);
            chk();
            // 三角形のリストを描画する。
            gl.drawElements(gl.TRIANGLES, pkg.VertexIndexBufferInf.value.length, gl.UNSIGNED_SHORT, 0);
            chk();
        }
        else {
            //  描画しない場合
            ...
            gl.bindTransformFeedback(gl.TRANSFORM_FEEDBACK, pkg.transformFeedback);
            chk();
            // すべてのvarying変数に対し
            for (var i = 0; i < pkg.varyings.length; i++) {
                var varying = pkg.varyings[i];
                // Transform Feedbackのバッファをバインドする。
                gl.bindBufferBase(gl.TRANSFORM_FEEDBACK_BUFFER, i, varying.feedbackBuffer);
                chk();
            }
            ...
            chk(); // TRIANGLES
            // 点ごとの描画をする。
            gl.drawArrays(gl.POINTS, 0, pkg.attribElementCount);
            chk();
            ...
            for (var i = 0; i < pkg.varyings.length; i++) {
                ...
                gl.getBufferSubData(gl.ARRAY_BUFFER, 0, varying.value);
                chk();
