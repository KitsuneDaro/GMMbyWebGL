...
class GMM {
    ...
    static CreateGMM(dist_n, data_n, x, regularation_value = 0.0, log_p_tolerance_value = 1e-6) {
        ...
        `;
        const regularation_value_string = regularation_value.toFixed(10);
        const sigma_shader = `
            uniform vec3 x[${data_n}];
            uniform vec3 mu[${dist_n}];
            uniform sampler2D gamma;
            ...
            void main() {
                ...
                for(int n = 0; n < ${data_n}; n++){
                    ...
                    sigma += gamma_n_m * vec3(
                        (x[n][k] - mu[m][k]) * (x[n][0] - mu[m][0]), (x[n][k] - mu[m][k]) * (x[n][1] - mu[m][1]),(x[n][k] - mu[m][k]) * (x[n][2] - mu[m][2]));
                }

                sigma /= gamma_sum[m];
                
                // regularation_value倍した単位行列を加算
                sigma[k] += ${regularation_value_string};
            }
        `;
        console.log(sigma_shader.split(/(\r\n|\r|\n)+/));
        const mu_pi_shader = `
            uniform vec3 x[${data_n}];
            uniform sampler2D gamma;
            uniform float gamma_sum[${dist_n}];
        ...
        console.log(i, norm_x_sum, log_p);
...
console.log(GMM.CreateGMM(dist_n, data_n, x));
